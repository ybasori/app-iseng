...
class Model {
    ...
    joinSqlQuery(joinArr: IJoin[], relationObj: IRelation | null, parentTable: string) {
        let sqlJoin = "";
    ...
    joinFilterSqlQuery(joinArr: IJoin[], relationObj: IRelation | null, parentTable: string): string[] {

        let filterQuery: string[] = [];

        for (let i = 0; i < joinArr.length; i++) {
            if (!!relationObj) {
                filterQuery = [...filterQuery, ...this.filterQuery(joinArr[i].filter, relationObj[joinArr[i].name].relatedTo.table)];
            }
        }
    ...
    }: { db?: string; table?: string; filter: IFilter; pagination?: IPagination; join?: IJoin[]; relations?: IRelation | null }):Promise<any[]> {
        return new Promise(async (resolve, reject) => {
            try {
                const dbi = !!alternativeDb ? alternativeDb : !!this.database ? this.database : "";
                const db = dbs[dbi as keyof typeof dbs];

                let filterQuery = this.filterQuery({ ...filter }, !!alternativeTable ? alternativeTable : (this.table ?? "") );
                let sortQuery = this.sortQuery(pagination);
                let paginationQuery = this.paginationQuery(pagination);

                let joinSql = join.filter(
                    (item) =>
                ...
                let joinQuery = this.joinSqlQuery(
                    joinSql,
                    !!alternativeRelations ? alternativeRelations : this.relations,
                    !!alternativeTable ? alternativeTable : (this.table ?? "") 
                );
                let selectJoinQuery = this.joinColumnSqlQuery(
                    joinSql,
                    !!alternativeRelations ? alternativeRelations : this.relations
                );
                let filterJoinQuery = this.joinFilterSqlQuery(
                    joinSql,
                    !!alternativeRelations ? alternativeRelations : this.relations,
                    !!alternativeTable ? alternativeTable : (this.table ?? "") 
                );
                ...
                const [mainResult] = await db.query(
                    `SELECT ${!!alternativeTable ? alternativeTable : this.table
                    }.* ${selectJoinQuery} FROM ${!!alternativeTable ? alternativeTable : this.table
                    } ${joinQuery} ${[...filterQuery, ...filterJoinQuery].length > 0 ? "WHERE" : ""} ${[...filterQuery, ...filterJoinQuery].join(" AND ")} ${sortQuery} ${paginationQuery}`
                );
                        ...
                        join.map((el) => {
                            ...
                            if (!!relations &&
                                ...
                                return this.getByFilter({
                                    db: relations[joinName].relatedTo.db,
                            ...
                            return new Promise((resolve: (arg0: never[][]) => any) => resolve([[]]));
                        })
                    );
    ...
    countByFilter(filter: any, join:IJoin[] = []) {
        const dbi = this.database ?? ""
        const db = dbs[dbi as keyof typeof dbs];

        let filterQuery = this.filterQuery({ ...filter }, this.table ?? "");
        ...
        return db.query(`SELECT COUNT(*) AS total FROM ${this.table} ${joinQuery} ${[...filterQuery, ...filterJoinQuery].length > 0 ? "WHERE" : ""} ${[...filterQuery, ...filterJoinQuery].join(" AND ")}`);
    }
    ...
    store(payload: any) {
        ...
        const values = [...Object.keys(payload).map((key)=>`'${payload[key]}'`), `'${new Date()}'`, `'${new Date()}'`].join(",")
    ...
    update(payload: any, filter: any) {
        ...
        const set = [...Object.keys(payload).map((key)=>`${key}='${payload[key]}'`)].join(",");
        const where = [...Object.keys(filter).map((key)=>`${key}='${filter[key]}'`)].join("AND");
